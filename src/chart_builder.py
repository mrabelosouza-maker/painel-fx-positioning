...
def make_dual_axis_chart(
    ...
    if fit_1a1:
        ...
        # doubleClick="autosize": com um range inicial cravado no eixo X, o
        # padrao ("reset+autosize") trava no reset e o duplo-clique nunca chega
        # a expandir — o refit dos eixos Y logo depois limpa o estado que o
        # Plotly usa para alternar entre os dois. Com "autosize" o duplo-clique
        # vai direto para a amostra toda e o "Reset axes" da modebar (a casinha)
        # devolve a janela inicial. Nada disso desenha no grafico, entao a
        # imagem exportada sai limpa.
        html = fig.to_html(
            full_html=False, include_plotlyjs=False, div_id=div_id,
            config=dict(doubleClick="autosize"),
        )
