...
def make_afp_hedge_ratio(wk: pd.DataFrame, title: str) -> str:
    """Fatia do fluxo spot que o forward hedgeia: semana a semana e acumulada.

    A linha acumulada e o que se le; as barras semanais estao la para mostrar a
    dispersao. As duas vao em eixos separados de proposito: a semanal vive entre
    -242% e +521% e a acumulada entre -86% e +96%, entao num eixo so a linha
    ficaria espremida em um quinto da altura do painel.
    """
    d = wk.dropna(subset=["hedge_wk", "hedge_cum"], how="all")
    if d.empty:
        return "<p>Dados indisponíveis</p>"
    ...
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Bar(
        x=x_str, y=100 * d["hedge_wk"], name="Semana (eixo dir.)",
        marker_color="#d1d5db",
        hovertemplate="semana de %{x}<br>%{y:+,.0f}%<extra></extra>",
    ), secondary_y=True)
    fig.add_trace(go.Scatter(
        x=x_str, y=100 * d["hedge_cum"], name="Acumulado desde jun/2022 (eixo esq.)",
        mode="lines", line=dict(color="#b45309", width=2.5),
        hovertemplate="semana de %{x}<br>acumulado: %{y:+,.0f}%<extra></extra>",
    ), secondary_y=False)

    wkv = (100 * d["hedge_wk"]).dropna()
    cumv = (100 * d["hedge_cum"]).dropna()
    # Eixo da linha: contem a serie inteira e sempre a faixa de 0 a 100%, que sao
    # as duas referencias que importam.
    c_lo = min(cumv.min() if len(cumv) else 0, -25)
    c_hi = max(cumv.max() if len(cumv) else 0, 125)
    c_pad = 0.10 * (c_hi - c_lo)
    # Eixo das barras: percentis, senao uma semana de denominador pequeno achata
    # todas as outras. As que saem da faixa ficam cortadas.
    b_lo = min(wkv.quantile(0.05) if len(wkv) else 0, -25)
    b_hi = max(wkv.quantile(0.95) if len(wkv) else 0, 125)
    b_pad = 0.10 * (b_hi - b_lo)
    ...
    fig.update_yaxes(
        title_text="Acumulado (% do spot hedgeado)", ticksuffix="%",
        range=[c_lo - c_pad, c_hi + c_pad], secondary_y=False,
    )
    fig.update_yaxes(
        title_text="Semana (%)", ticksuffix="%", showgrid=False,
        range=[b_lo - b_pad, b_hi + b_pad], secondary_y=True,
    )
    fig.add_hline(y=100, line_dash="dash", line_color="green", line_width=1,
                  secondary_y=False)
    fig.add_hline(y=0, line_color="black", line_width=0.8, secondary_y=False)
    fig.add_annotation(
